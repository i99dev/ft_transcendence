      ...
      <div class="centered flex-col h-screen w-full">
        <div class="centered flex-col" style="max-width: 80vw;">
          <h1 class="text-white text-xl md:text-2xl lg:text-3xl text-center capitalize p-2 mb-1">
            Hi <span class="capitalize m-2 text-white p-2 rounded-xl bg-primary">{{ useRoute().query.login }}</span>
          </h1>
          ...
          <h3 v-if="timer" class="text-white text-md md:text-lg lg:text-xl text-center normal-case mb-5 p-2">
            code will expire in {{ Math.trunc(timer / 60) }} minutes
          </h3>
          <h3 v-else class="text-white text-md md:text-lg lg:text-xl text-center normal-case mb-5 p-2">
            code is expired
          </h3>
...
const submitCode = async (code: string) => {
  ...
  if (login) {
    ...
    if (data.value) {
      ...
      toast.add({ severity: 'error', summary: 'Incorrect', detail: 'Incorrect verification code' })
    }
  }
}
...
const resendCode = async () => {
  ...
  if (login) {
    ...
    if (data.value)
      ...
      toast.add({ severity: 'success', summary: 'Valid', detail: 'Correct verification code' })
      timerCountdown()
  }
}
