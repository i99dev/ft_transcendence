<template>
    <div class="no-context-menu">
        <div class="fixed top-4 right-8 cursor-pointer w-8 h-8 z-50">
            <button v-if="muteSound" @click="handleMuteSound"
            ...
            <button v-else @click="handleMuteSound"
                class="relative hover:bg-primary rounded-full smooth-transition p-2 w-12 aspect-square">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
        ...
        <div>
            <GameClosePopup v-if="exit" @closePopup="switchExistStatus(false)" @GiveUp="exitGame"
                detail="You will be considerd a leaver!" confirmation="Are you sure you want to exit the game?" />
            <div
            ...
            <GameResult v-if="gameResult" @vue-mounted="exit = false" :gameResultMessage="gameResultMessage"
                @playAgain="playAgain" class="z-20" />
        </div>
        <div v-show="showRotateOverlay"
