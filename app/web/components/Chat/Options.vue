<template>
    <div class="centered">
        <TransitionRoot :show="groupChatSearching" enter="transition-opacity duration-500 ease-in-out"
            enter-from="opacity-0" enter-to="opacity-100" leave="transition-opacity duration-500 ease-in-out"
            leave-from="opacity-100" leave-to="opacity-0" class="w-4/5 mx-2 flex items-center">
            <input v-model="searchedGroupChats" @input="setSearchedGroupChats(searchedGroupChats)"
                ...
                type="text" ref="chatSearch" placeholder="Search" @focus-visible="!chatType" />
        </TransitionRoot>

        <button @click="($event: any) => handleChatSearch($event)"
