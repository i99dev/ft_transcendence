    ...
    <div class="border-y border-white h-90vh overflow-hidden">
        <div id="chat-list" class="overflow-y-scroll h-full">
            ...
            <div class="flex flex-col" x-descriptions="Tab component">
                <TransitionRoot appear :show="isJoinGroupChatOpened" as="template">
                    <Dialog as="div" @close="closejoinGroupPasswordPopup()" class="relative z-10">
                        ...
                        <div class="fixed inset-0">
                            <div class="absolute w-80 top-1/4 right-10 min-h-full items-center justify-end p-4 text-center">
                                        ...
                                        <form @submit.prevent="" class="flex">
                                            <div class="relative mx-2">
                                                ...
                                                <input
                                                    ...
                                                    id="joinGroupPassword" type="password" v-model="joinGroupPassword"
                ...
                <!-- chat element -->
                <button v-for="chat in chats" :key="chat?.id" @click="setCurrentChat(chat)"
                    ...
                    @mouseover="hoverButton = chat" @mouseleave="hoverButton = null">
                    <div class="relative" style="width: 10%">
                        <img v-if="chatType === 'DM'" :src="chat?.users[0].image" alt="User Photo"
                            class="rounded-full w-10 h-10 object-cover" />
                        <img v-else :src="chat?.image" alt="User Photo" class="rounded-full w-10 h-10 object-cover" />
                        <!-- online badge -->
                        <UserProfileStatus v-if="chatType === 'DM'" :status="chat?.users[0].status"
                    ...
                    <div class="flex justify-between" style="width: 90%">
                        <div class="flex flex-col mx-4 w-1/2">
                            ...
                            <div v-else class="flex justify-start text-white"
                            ...
                            <div v-if="chatType"
                                ...
                                <span class="inline-block max-w-full overflow-hidden whitespace-nowrap text-ellipsis">
                                    {{ chat?.chat_room.messages[0]?.content }}
                                </span>
                            </div>
                        </div>
                        <!-- Invite Game Button -->
                        <button v-if="(chatType === 'DM' || chatType === null) && hoverButton === chat"
                            ...
                            <svg v-if="chatType === 'DM'" xmlns="http://www.w3.org/2000/svg"
                                class="w-5 h-5 stroke-white stroke-2 fill-none" viewBox="0 0 24 24">
            ...
            <div v-if="chatType === 'GROUP'">
                ...
                <ChatCreateGroup :isOpened="isChatCreateGroupOpened"
