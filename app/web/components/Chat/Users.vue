<template>
    <MainPopup :show="props?.show" @closeMainPopup="closePopup">
        <div v-for="user in users" :key="user.username" class="flex-row inline-flex flex-nowrap">
            <button class="border rounded-full bg-background ease-in-out transition duration-200 m-2 relative"
                @click="removeUser(user)">
                <img class="rounded-full w-8 h-8 object-cover" :src="user.image" :alt="user.username" />
                <div
        ...
        <div class="flex justify-end mt-2">
            <button v-if="props.cancelButton"
                ...
                type="button" @click="closePopup('cancel')">
                {{ props.cancelButton || 'cancel' }}
            </button>
            <button class="flex-shrink-0 bg-secondary hover:bg-primary text-white py-1 px-2 rounded capitalize"
                type="button" @click="sendUsersList">
                {{ props.addButton || 'add' }}
            </button>
        </div>
    </MainPopup>
