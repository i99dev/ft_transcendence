            ...
            <div
                        ...
                        <div class="flex flex-row items-center w-full justify-between">
                            ...
                            <div class="relative">
                                ...
                                <div v-if="showstat"
                                    ...
                                    <span
                                        class="block px-4 py-2 text-sm text-gray-700 transition duration-150 ease-in-out">XP
                                        level: {{ userData.xp }}</span>
                                    <span
                                        class="block px-4 py-2 text-sm text-gray-700 transition duration-150 ease-in-out">Winning
                                        Rate: {{ (WinRate * 100).toFixed(2) }}%
                                    </span>
                                    <span
                                        class="block px-4 py-2 text-sm text-gray-700 transition duration-150 ease-in-out">Total
                                        Wins: {{ totalWins }}
                                    </span>
                                    <span
                                        class="block px-4 py-2 text-sm text-gray-700 transition duration-150 ease-in-out">Total
                            ...
                            <div class="flex justify-center" @click="updateUsername" v-if="!editBoolaen">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                    stroke="currentColor" class="min-w-full h-8 bg-green-300">
                ...
                <div v-if="isMe && !isProfile" class="flex flex-row space-x-6">
                    <div class="relative cursor-pointer" @click="openChatModel">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                            stroke="currentColor" class="w-8 h-8 mt-2">
                            <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <div @click="openFriendsModel" class="relative cursor-pointer">
                        ...
                        <!-- badge offline -->
                        <div class="absolute -top-1 -right-1 flex items-center justify-center rounded-full w-3.5 h-3.5 p-2.5"
                            :class="{
                    ...
                    <button @click="useLogout" class="cursor-pointer relative rounded-full">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-8 h-8 stroke-1 stroke-current" viewBox="0 0 24 24"
                            fill="none">
                    ...
                    <button @click="updateTwoFacAuth" class="cursor-pointer relative rounded-full">
                        <svg v-if="user_info.two_fac_auth" xmlns="http://www.w3.org/2000/svg"
                            class="w-8 h-8 fill-white stroke-black stroke-1" viewBox="0 0 24 24" stroke-linecap="round"
                        ...
                        <svg v-else xmlns="http://www.w3.org/2000/svg" class="w-8 h-8 fill-white stroke-black stroke-1"
                    ...
                    <button @click="navigateTo('/help')" class="cursor-pointer relative rounded-full">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="50" fill="currentColor"
                ...
                <div v-else-if="!isMe" class="flex space-x-6">
                    <button @click="useDMUser(userData.login)"
                    ...
                    <button @click="addFriend(username)" :title="'Add to friend list'"
                        class="p-2 border-y border-slate-100 bg-slate-100 rounded-full relative mb-1 focus:outline-indigo-400 focus:-outline-offset-2">
