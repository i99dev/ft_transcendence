<template>
    <div v-if="props.isMe && !props.isProfile" class="flex flex-row space-x-6">
        ...
        <button v-click-effect:logout="useLogout"
            class="relative hover:bg-primary rounded-full smooth-transition p-2 w-12 aspect-square">
...
const muteSound = ref(false as boolean)
...
function handleMuteSound () {
    ...
    console.log('mute', muteSound.value, 'pause', isPaused('login'))
    if (isPaused('login') && muteSound.value)
        play('login')
    else if (!isPaused('login') && muteSound)
        pause('login')
